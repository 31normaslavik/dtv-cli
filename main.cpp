...
int main(int argc, char** argv) try {
    ...
    std::string path_argv{argv[2]};
    fs::path path_to_save;

    if(path_argv.starts_with('.') && path_argv.size() == 1){
        path_to_save = std::getenv("PWD");
    }else
    if(path_argv.starts_with("./")){
        path_to_save = std::string(std::getenv("PWD")) + "/" + path_argv.substr(2);
    }else
    if(!path_argv.starts_with("/")){
        path_to_save = std::string(std::getenv("PWD")) + "/" + path_argv;
    }else
    {
        path_to_save = path_argv;
    }

    if(path_to_save.empty())
        throw std::runtime_error("Path to save file is empty");
        ...
        for(auto&i : playlist){
            ...
            for (const auto &entry : fs::directory_iterator(tmp)) {
                if (entry.is_regular_file() && entry.path().string().find(i.display_id) != std::string::npos) {
                    ...
                    switch (extension(entry.path().extension().string())) {
                    case EXTENSION::MP4:
                    case EXTENSION::WEBM:
                        video = entry.path().filename().string();
                        break;
                    case EXTENSION::MP3:
                        translated_audio = entry.path().filename().string();
                        break;
                    case EXTENSION::M4A:
                        audio = entry.path().filename().string();
                    default:
                        break;
                    }
                }
            ...
            if(ffmpeg_command_run(video, translated_audio, audio, i.display_id + ".mkv") != 0)
            ...
            for(std::string temp, newstring ; ss >> temp;){
                ...
                for(auto pos = temp.find_first_of(exclude_chars); pos != std::string::npos ; pos = temp.find_first_of(exclude_chars) ){
                    temp.erase(pos,1);
                }

                newstring += temp.append("_");
                i.title = newstring;
                i.title.erase(i.title.size() - 1);
            }
            if(fs::exists(i.display_id + ".mkv")) {
                cout<< tmp.string() << " " << path_to_save.string() << endl;
                if(fs::copy_file(tmp / std::string(i.display_id + ".mkv"), path_to_save / std::string(
                                                                                (i.playlist_index == 0 ? "" : std::to_string(i.playlist_index) + "_") + i.title + ".mkv"),
